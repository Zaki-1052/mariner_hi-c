#!/usr/bin/env Rscript
# scripts/stripe_body_gene_enrichment.R
#
...
ANNOT_FILE <- sprintf("stripes/stripenn/outputs/%s/visualizations/%s_annotated_stripes.tsv",
                     TP_ID, TP_ID)
OUTPUT_DIR <- sprintf("outputs/stripe_integration/%s/body_gene_enrichment", TP_LABEL)
...
explode_body <- function(df) {
  ...
  out <- df %>%
    select(stripe_id, direction, direction_confidence, significant_FDR05, body_genes) %>%
