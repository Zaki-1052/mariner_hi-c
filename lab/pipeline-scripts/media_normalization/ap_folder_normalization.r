...
dir<-"/media/rs_256/normalization/250620_ap_h2ak119ub_rnorm_6reps/"
cat("workdir: ", dir, "\n")

# Automatically list all .bw files in the directory
bw_files <- list.files(path = dir, pattern = "\\.bw$", full.names = TRUE)

# Extract file names without the directory path and extension for further processing
samples <- gsub(pattern = ".bw$", replacement = "", x = basename(bw_files))
