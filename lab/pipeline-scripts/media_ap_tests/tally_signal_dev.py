...
def merge_with_bed_file(folder_path, output_file):
    ...
    bed_df = pd.read_csv(bed_file, sep='\t', comment='#', header=None)

    coordinate_df = bed_df.iloc[:, [0, 1, 2, 3]].copy()
    coordinate_df['Coordinate'] = (
        coordinate_df.iloc[:, 0] + ":" + 
        (coordinate_df.iloc[:, 1]).astype(str) + "-" + 
        coordinate_df.iloc[:, 2].astype(str)
    )
    coordinate_df['GeneName'] = coordinate_df.iloc[:, 3]
    ...
    output_df['GeneName'] = coordinate_df['GeneName'].values
    columns = output_df.columns.tolist()

    output_df = output_df[columns]

    # Move the 'Coordinate' column to the leftmost position
  #  cols_order = ['Coordinate'] + [col for col in output_df.columns if col not in ['Coordinate']]
    cols_order = ['GeneName', 'Coordinate'] + [col for col in output_df.columns if col not in ['GeneName', 'Coordinate']]
    output_df = output_df[cols_order]

    # split rows with mutliple genes into their own row
    output_df['GeneName'] = output_df['GeneName'].astype(str).str.split(',')
    output_df = output_df.explode('GeneName')
    output_df['GeneName'] = output_df['GeneName'].str.strip()
    ...
    #agg_df = output_df.groupby("GeneName", as_index=False).sum(numeric_only=True)
    #cols = agg_df.columns.tolist()
    #agg_cols_order = ['GeneName'] + [col for col in cols if col != 'GeneName']
    #agg_df = agg_df[agg_cols_order]

    #aggregated_file = os.path.join(folder_path, f"{os.path.basename(folder_path.rstrip('/'))}_aggregated.xlsx")
    #agg_df.to_excel(aggregated_file, index=False)
    #print(f"Aggregated file has been saved to {aggregated_file}")
