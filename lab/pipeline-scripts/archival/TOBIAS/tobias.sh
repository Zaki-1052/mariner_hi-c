...
# Make output directory for bias-corrected files
mkdir -p ${OUTPUT_DIR}/atacorrect/ctrl \
	${OUTPUT_DIR}/atacorrect/mut

# Correct bias for ctrl
TOBIAS ATACorrect \
    --bam $CTRL_BAM \
    --genome $GENOME_FASTA \
    --peaks $CTRL_PEAKS \
    --blacklist $BLACKLIST_BED \
    --outdir ${OUTPUT_DIR}/atacorrect/ctrl \
    --cores 14
 
# Correct bias for mut
TOBIAS ATACorrect \
    --bam $MUT_BAM \
    --genome $GENOME_FASTA \
    --peaks $MUT_PEAKS \
    --blacklist $BLACKLIST_BED \
    --outdir ${OUTPUT_DIR}/atacorrect/mut \
    --cores 14
