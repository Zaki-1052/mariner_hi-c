...
>&2 date
$javabin/java -jar $trimmomaticbin/$trimmomaticjarfile PE -threads 16 -phred33 $dirname/"$base"_R1.fastq.gz $dirname/"$base"_R2.fastq.gz $trimdir/"$base"_1.paired.fastq.gz $trimdir/"$base"_1.unpaired.fastq.gz $trimdir/"$base"_2.paired.fastq.gz $trimdir/"$base"_2.unpaired.fastq.gz ILLUMINACLIP:$adapterpath/Truseq3.PE.fa:2:15:4:4:true LEADING:20 TRAILING:20 SLIDINGWINDOW:4:15 MINLEN:25
...
>&2 date
$bowtie2bin/bowtie2 -p 16 --dovetail --phred33 -x $bt2idx/%s -1 $trimdir2/"$base"_1.paired.fastq.gz -2 $trimdir2/"$base"_2.paired.fastq.gz > $aligndir/"$base"_aligned_reads.sam 2> $logdir/"$base".bowtie2 
...
$memebin/meme-chip -oc $meme_outdir -dreme-m %d -meme-nmotifs %d -meme-p 8 -spamo-skip -fimo-skip $mpaddedfa/$summitfa
...
base=`basename $relsample _R1.fastq.gz`
...
if __name__=="__main__":
	...
	for filename in os.listdir(config["input/output"]["fastq_directory"]):
		if filename.endswith("fastq.gz"):
			flist.add(filename)
	for x in flist:
		m1 = re.match("(.*)_R1.fastq.gz", x)
		m2 = re.match("(.*)_R2.fastq.gz", x)
		if m1 is None and m2 is None:
			print "Error:", x, " no pattern of _R1.fastq.gz, or _R2.fastq.gz detected"
			sys.exit(1)
		if m1 is not None:
			x2 = m1.group(1) + "_R2.fastq.gz"
			if not x2 in flist:
				print x, "does not have a corresponding _R2.fastq.gz file"
				sys.exit(1)
		if m2 is not None:
			x2 = m2.group(1) + "_R1.fastq.gz"
			if not x2 in flist:
				print x, "does not have a corresponding _R1.fastq.gz file"
				sys.exit(1)
