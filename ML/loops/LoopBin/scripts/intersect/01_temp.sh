...
awk -v gap=8000 '{print $1,($2-gap < 0 ? 0 : $2-gap), $3+gap, $11, NR}' OFS="\t" "$file2" > "$ol"temp/control_anchor1.bed
awk -v gap=8000 '{print $4, ($5-gap < 0 ? 0 : $5-gap), $6+gap, $11, NR}' OFS="\t" "$file2" > "$ol"temp/control_anchor2.bed
awk  'BEGIN {OFS=FS="\t"}
  {
    print $1, $2, $3, $4, $5, $6, $9, NR
  }' $file1 > "$ol"temp/degron.bedpe
# find overlap with two anchors
bedtools pairtobed -type either -a "$ol"temp/degron.bedpe -b "$ol"temp/control_anchor1.bed > "$ol"temp/overlap_degron_anchor1.bed
bedtools pairtobed -type either -a "$ol"temp/degron.bedpe -b "$ol"temp/control_anchor2.bed > "$ol"temp/overlap_degron_anchor2.bed

# Combine overlaps from both anchors into BEDPE format with merged labels
awk 'BEGIN {OFS=FS="\t"}
  NR==FNR {dic[$8"|"$13]=0; next} $8"|"$13 in dic {print $1, $2, $3, $4, $5, $6, $12, $7}' "$ol"temp/overlap_degron_anchor1.bed "$ol"temp/overlap_degron_anchor2.bed | sort -u > "$ol"degron_intersect.bedpe