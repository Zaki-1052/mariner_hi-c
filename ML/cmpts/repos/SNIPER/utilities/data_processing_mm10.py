# ML/cmpts/repos/SNIPER/utilities/data_processing_mm10.py
# mm10-adapted copy of data_processing.py for SNIPER.
import os
import numpy as np
...
def hicToMat(h,juicer_cmd,tmp_dir='.',prefix='hic',autoremove=False,overwrite=False,save_matrix=False,verbose=False):
	try:
		os.stat(tmp_dir)
	except:
		os.mkdir(tmp_dir)
	...
	for chrm1 in range(1,20,2):
		for chrm2 in range(2,20,2):
...
def getColorString(n):
    subcColors = ['34,139,34','152,251,152','220,20,60','255,255,0']
    return subcColors[n]
...
def predictionsToBed(path, odds, evens, cropMap, res=100000, sizes_file='data/mm10.chrom.sizes'):
	rowMap = cropMap['rowMap'].astype(int)
	colMap = cropMap['colMap'].astype(int)
