	...
	{	
		...
		if(!is.null(contact_file_hic)) ## if contact matrix in hic format is provided
		{
			...
			if(inherits(contact_mat_raw, 'try-error') || (!inherits(contact_mat_raw, 'try-error') && nrow(na.omit(contact_mat_raw)) < 100)) contact_mat_raw = try(strawr::straw("VC_SQRT", contact_file_hic, as.character(chr2query), as.character(chr2query), "BP", bin_size_input))
			if(inherits(contact_mat_raw, 'try-error') || (!inherits(contact_mat_raw, 'try-error') && nrow(na.omit(contact_mat_raw)) < 100)) contact_mat_raw = try(strawr::straw("VC", contact_file_hic, as.character(chr2query), as.character(chr2query), "BP", bin_size_input))
			if(inherits(contact_mat_raw, 'try-error') || (!inherits(contact_mat_raw, 'try-error') && nrow(na.omit(contact_mat_raw)) < 100)) stop(sprintf('Your provided hic file does not contain information given the bin_size=%s and any of the normalization method KR/VC/VC_SQRT', bin_size_input))	
