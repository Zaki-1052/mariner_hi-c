#!/usr/bin/env Rscript
# scripts/stripe_compartment_crossref.R
#
...
# Late and early compartment tables are in different subdirectories
PC_BASE <- if (TP_LABEL == "late") {
  "tads/tad-pc-analysis/output/compartment_analysis"
} else {
  "tads/tad-pc-analysis/output/compartment_analysis_early"
}
...
OUTPUT_DIR <- sprintf("outputs/stripe_integration/%s/compartment_crossref", TP_LABEL)
...
tests_long <- do.call(rbind, lapply(tests, function(x) {
  data.frame(test = x$test,
             details = paste(setdiff(colnames(x), c("test", "p_value")),
                             unlist(lapply(setdiff(colnames(x), c("test", "p_value")),
                                           function(k) format(x[[k]], digits = 4))),
                             sep = "=", collapse = "; "),
             p_value = x$p_value,
             stringsAsFactors = FALSE)
}))
