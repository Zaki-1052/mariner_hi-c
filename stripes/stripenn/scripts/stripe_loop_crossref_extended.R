#!/usr/bin/env Rscript
# stripes/stripenn/scripts/stripe_loop_crossref_extended.R
#
...
LOOPS_FILE <- if (TP_LABEL == "late") {
  file.path(REPO_ROOT, "outputs/250402-late_outputs/merged_loops/characterized_loops.tsv")
} else {
  file.path(REPO_ROOT, "outputs/250831-early_outputs/merged_loops/characterized_loops.tsv")
}
OUTPUT_DIR <- file.path(REPO_ROOT, sprintf("outputs/stripe_integration/%s/loop_crossref", TP_LABEL))
