...
mkdir -p \
  cluster/data \
  cluster/scripts \
  cluster/bap1_late/cluster3 \
  cluster/bap1_late/figures/annotation \
  cluster/bap1_late/figures/ChIP_intersect \
  cluster/bap1_late/figures/deeptools \
  cluster/bap1_late/figures/deeptools_input \
  cluster/bap1_late/chromHMM/binarized \
  cluster/bap1_late/chromHMM/learned_model \
  cluster/bap1_late/chromHMM/anchor_input \
  cluster/bap1_late/chromHMM/span_input \
  cluster/bap1_late/chromHMM/peak_beds \
  cluster/bap1_late/cooltools

echo ""
echo "[b] Step 1.1 — Building loop count file..."
"${CLUSTER_PY}" cluster/scripts/01_build_loop_count_file.py
py_status=$?
...
"${SYS_RSCRIPT}" cluster/scripts/02_build_mm10_gene_annotation.R
r_status=$?
...
echo "============================================================"
for f in cluster/data/late_merged_loop_counts.txt \
         cluster/data/late_merged_loop_metadata.tsv \
         cluster/data/mm10_knownGene_pp.bed; do
  if [ -s "${f}" ]; then
    rows=$(wc -l < "${f}")
