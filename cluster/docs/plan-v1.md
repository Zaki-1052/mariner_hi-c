...
**Status:** All 8 sub-analyses (4.1–4.8) executed end-to-end via `LOG=cluster/phase4.txt bash cluster/scripts/run_phase4.sh all` in ~6 min on Mac. KEY result (4.4) shows clean differential ChromHMM enrichment distinguishing two mechanisms in the same paper: gained loops (clust5) have Polycomb at BOTH anchor (6.59×) AND span (3.03×) — Polycomb-Polycomb domain expansion / extrusion impediment; lost loops (clust6) have Polycomb at anchor (2.09×) but NOT span (0.94×) — anchor-disruption / sensitivity model. **See `cluster/plan-p2.md` for the full Phase 4 — DONE record** (corrections-during-execution, per-sub-analysis verification with stats/biology, file inventory). Run log captured at `cluster/phase4.txt`.

**Top-level corrections** (full list in plan-p2.md):
- `multi_format_savefig` now patches `matplotlib.figure.Figure.savefig` (not just `plt.savefig`) — covers `plotting.stacked()` which uses `fig.savefig` directly.
- All BigWigs sourced from `/Users/zakiralibhai/sdsc/bigwigs/` (mariner_hi-c `peaks/bigwigs/macs2.narrow.aug18.dedup` mut files are 0-byte). Phase 5 should follow.
- Anchor-side dedup before pyBigWig query in 4.3 (hub-anchor weighting).
- 'U' → 'E' regex fix and `sig_chr1`→`sig_chr2` typo fix in adapted Popay code.
- DiffBind columns renamed `Peak_*`→`*` once at load to avoid scattered downstream edits.
