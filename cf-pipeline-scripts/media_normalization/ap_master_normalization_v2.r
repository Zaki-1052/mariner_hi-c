...
dir<-"/data/rs_256/workdir/aligned.aug10/"
cat("workdir: ", dir, "\n")

# Get BigWig names from command-line arguments
args <- commandArgs(trailingOnly = TRUE)
num_bw <- length(args)
samples <- args

# Print parsed inputs for debugging
cat("Number of BigWigs: ", num_bw, "\n")
cat("BigWig samples:\n")
cat(samples, sep = "\n")
...
masked<-read.table("/media/rs_256/normalization/master_files/manual.mask.ultimate.bed",sep="\t",header=FALSE,stringsAsFactors=FALSE) # the regions we want to avoid
#convert to dx units
