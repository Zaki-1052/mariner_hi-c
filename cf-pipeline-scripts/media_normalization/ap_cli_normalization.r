...
# Args: <pattern> [old_dir] [new_dir]
args <- commandArgs(trailingOnly = TRUE)
pattern <- if (length(args) >= 1) args[[1]] else stop("Usage: Rscript normalization.R <pattern> [old_dir] [new_dir]")
old_dir <- if (length(args) >= 2) args[[2]] else "ap_adme_bw"
new_dir <- if (length(args) >= 3) args[[3]] else "ap_adme_new_2rep_bw"

# keep a workdir print for continuity
dir_print <- paste(old_dir, new_dir, sep = ", ")
cat("workdir: ", dir_print, "\n")
cat("pattern: ", pattern, "\n")
dir <- ""
# list exactly the 8 target files: EARLY/LATE with 1–2 from old and 3–4 from new
rx_old <- paste0("^", pattern, "(EARLY|LATE)(1|2)\\.bw$")
rx_new <- paste0("^", pattern, "(EARLY|LATE)(3|4)\\.bw$")
bw_files <- c(
  list.files(path = old_dir, pattern = rx_old, full.names = TRUE),
  list.files(path = new_dir, pattern = rx_new, full.names = TRUE)
)

# Extract names without path/extension
samples <- tools::file_path_sans_ext(bw_files)

cat("Loaded BigWig files:\n")
cat(samples, sep = "\n")
