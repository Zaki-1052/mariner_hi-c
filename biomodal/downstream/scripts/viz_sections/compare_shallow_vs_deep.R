...
# Standalone comparison: shallow-seq (run-2) vs deep-seq (run-3)
...
# Source shared config for packages, helpers, colors, and deep-seq DATA_PATHS
source("scripts/viz_sections/_shared_config.R")

cat("================================================================================\n")
cat("SHALLOW-SEQ (RUN-2) vs DEEP-SEQ (RUN-3) COMPARISON\n")
cat("================================================================================\n\n")

# =============================================================================
# SHALLOW-SEQ PATHS (run-2, hardcoded timestamps)
# =============================================================================
...
SHALLOW_PATHS <- list(
  ...
  cpg_islands_hmc = file.path(BASE_DIR, "modality/outputs/run-2/outputs_CG/Results/gencode.vM25.mouse.cpg_islands.annotation/DMR_20260121_171455/DMR_hmc_control__mutant_20260121_171455.bed"),
  cpg_shores_hmc = file.path(BASE_DIR, "modality/outputs/run-2/outputs_CG/Results/gencode.vM25.mouse.cpg_shores.annotation/DMR_20260121_171910/DMR_hmc_control__mutant_20260121_171910.bed"),
  cpg_shelves_hmc = file.path(BASE_DIR, "modality/outputs/run-2/outputs_CG/Results/gencode.vM25.mouse.cpg_shelves.annotation/DMR_20260121_171718/DMR_hmc_control__mutant_20260121_171718.bed"),
  promoters_hmc = file.path(BASE_DIR, "modality/outputs/run-2/outputs_CG/Results/gencode.vM25.mouse.promoters.annotation/DMR_20260121_172220/DMR_hmc_control__mutant_20260121_172220.bed"),
  tss_hmc = file.path(BASE_DIR, "modality/outputs/run-2/outputs_CG/Results/gencode.vM25.mouse.tss_region.annotation/DMR_20260121_172333/DMR_hmc_control__mutant_20260121_172333.bed")
)

# Deep-seq paths come from DATA_PATHS (loaded via _shared_config.R, now run-3)
...
cat("Validating shallow-seq (run-2) paths...\n")
for (name in names(SHALLOW_PATHS)) {
  if (!file.exists(SHALLOW_PATHS[[name]])) {
    stop(sprintf("Shallow-seq file missing: %s\n  Path: %s", name, SHALLOW_PATHS[[name]]))
  }
}
cat("  All shallow-seq paths validated.\n")

cat("Validating deep-seq (run-3) paths...\n")
...
for (name in deep_dmr_keys) {
  if (!file.exists(DEEP_PATHS[[name]])) {
    stop(sprintf("Deep-seq file missing: %s\n  Path: %s", name, DEEP_PATHS[[name]]))
  }
}
cat("  All deep-seq paths validated.\n\n")

# Color palette for run comparison
RUN_COLORS <- c("Shallow (run-2)" = "#FF8C00", "Deep (run-3)" = "#1E90FF")
...
dmr_counts$Run <- factor(dmr_counts$Run, levels = c("Shallow (run-2)", "Deep (run-3)"))
...
p_counts <- ggplot(dmr_counts, aes(x = Region, y = Significant, fill = Run)) +
  ...
  labs(
    title = "Significant DMRs: Shallow-Seq vs Deep-Seq",
...
p_status <- ggplot(status_summary, aes(x = modification, y = n, fill = status)) +
  ...
  labs(
    title = "Gene Status: Shallow-Seq vs Deep-Seq",
    subtitle = "How gene significance changed with deeper sequencing",
...
p_effect <- ggplot(scatter_data,
  ...
  labs(
    title = "Effect Size Stability: Shallow vs Deep Sequencing",
    subtitle = sprintf("5mC r=%.3f | 5hmC r=%.3f | Dashed line = identity",
                       mc_cor, hmc_cor),
    x = "Shallow-Seq Effect Size (%)",
    y = "Deep-Seq Effect Size (%)"
  ) +
  theme_biomodal()
...
p_delta_hist <- ggplot(qval_data, aes(x = delta_q, fill = modification)) +
  ...
  labs(
    title = "Change in Q-values: Deep - Shallow",
    subtitle = sprintf("%.1f%% of genes improved (negative = better in deep-seq)", pct_improved),
    x = "Delta Q-value (deep - shallow)",
    y = "Number of Genes"
  ) +
  theme_biomodal()
...
p_nlog10 <- ggplot(qval_data, aes(x = shallow_nlog10, y = deep_nlog10)) +
  ...
  labs(
    ...
    subtitle = "Points above diagonal = more significant in deep-seq",
    x = "-log10(q) Shallow-Seq",
    y = "-log10(q) Deep-Seq"
  ) +
  theme_biomodal()
...
p_qval_combined <- p_delta_hist / p_nlog10 +
  plot_annotation(
    title = "Statistical Power Improvement with Deeper Sequencing",
    theme = theme(plot.title = element_text(hjust = 0.5, face = "bold", size = 16))
  )

save_multiformat_ggplot(p_qval_combined, file.path(COMP_DIR, "20e_qvalue_improvement"),
                        width = 12, height = 12)
cat(sprintf("  %.1f%% of genes have improved q-values in deep-seq\n", pct_improved))
...
p_coverage <- ggplot(cov_data, aes(x = shallow_coverage, y = deep_coverage)) +
  ...
  scale_y_log10() +
  labs(
    title = "Coverage Comparison: Shallow vs Deep Sequencing (5mC)",
    subtitle = sprintf("Median coverage: %.1f (shallow) vs %.1f (deep) | r=%.3f",
                       median_shallow, median_deep, cov_cor),
    x = "Mean Coverage (Shallow-Seq, log10)",
    y = "Mean Coverage (Deep-Seq, log10)"
  ) +
  theme_biomodal()

save_multiformat_ggplot(p_coverage, file.path(COMP_DIR, "20f_coverage_comparison"),
                        width = 10, height = 9)
cat(sprintf("  Median coverage: %.1f (shallow) vs %.1f (deep)\n",
...
venn_list <- list(
  "Shallow (run-2)" = shallow_coord_genes,
  "Deep (run-3)" = deep_coord_genes
)
...
cat("Key findings:\n")
cat(sprintf("  mC significant: %d (shallow) -> %d (deep)\n",
            sum(shallow_mc$significant), sum(deep_mc$significant)))
cat(sprintf("  hmC significant: %d (shallow) -> %d (deep)\n",
...
cat(sprintf("  Coordinated pattern: %.1f%% (shallow) -> %.1f%% (deep)\n",
            100 * mean(shallow_coord$coordinated),
            100 * mean(deep_coord$coordinated)))
cat(sprintf("  Coverage increase: %.1fx -> %.1fx median\n",
            median_shallow, median_deep))
cat("\n")
