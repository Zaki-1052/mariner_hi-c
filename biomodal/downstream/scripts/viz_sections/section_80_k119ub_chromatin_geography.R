# scripts/viz_sections/section_80_k119ub_chromatin_geography.R
# Section 80: H2AK119ub Drives Hypermethylation at Active Chromatin
#
# Consolidated section drawing from sections 10, 17, 29, 30, 33.
#
# Panels:
#   A  Sec 10  Stacked fraction bar (chromatin_state_summary.tsv)
#   B  Sec 33  Forest plot (diffbind_logistic_model_coefficients.tsv)
#   C  Sec 29  Compartment enrichment (compartment_fisher_tests.tsv)
#   D  Sec 30  Polycomb exclusion (polycomb_fisher_tests.tsv)
#   E  Sec 17  K119ub honest breakdown (k119ub_honest_breakdown.tsv)
#
# Output: plots/visualizations/80_k119ub_chromatin_geography/
# =============================================================================
...
SEC80_DIR <- file.path(OUTPUT_DIR, "80_k119ub_chromatin_geography")
dir.create(SEC80_DIR, recursive = TRUE, showWarnings = FALSE)

save_plot <- function(p, name, w = 10, h = 7) {
  save_multiformat_ggplot(p,
    base_path = file.path(SEC80_DIR, name),
...
build_panel_C <- function() {
  ...
  ggplot(cf_use, aes(x = odds_ratio, y = label, fill = target)) +
    ...
    geom_errorbar(aes(xmin = ci_lower, xmax = ci_upper),
                  width = 0.2, linewidth = 0.4, colour = "grey25", orientation = "y") +
...
# COMPOSE + SAVE
# =============================================================================
message("Building Section 80 panels ...")
pA <- build_panel_A()
pB <- build_panel_B()
pC <- build_panel_C()
pD <- build_panel_D()
...
save_plot(pA, "80a_chromatin_state_direction", w = 8, h = 6)
save_plot(pB, "80b_logistic_forest_plot", w = 8, h = 6)
save_plot(pC, "80c_compartment_enrichment", w = 8, h = 6)
save_plot(pD, "80d_polycomb_exclusion", w = 8, h = 6)
save_plot(pE, "80e_k119ub_honest_breakdown", w = 10, h = 6)

sec80_design <- "AABB\nCCDD\n#EE#"

sec80_composite <- (pA + pB + pC + pD + pE) +
  plot_layout(
    design  = sec80_design,
    heights = c(1.2, 1, 0.7)
  ) +
  plot_annotation(
    tag_levels = "A",
    title = "Section 80: H2AK119ub drives hypermethylation at active chromatin",
    theme = theme(
      plot.title = element_text(face = "bold", size = 11, hjust = 0)
    )
  )

save_plot(sec80_composite, "80_composite", w = 14, h = 12)

message("Section 80 complete.")
