# scripts/viz_sections/section_81_tet_mechanism.R
# =============================================================================
...
SEC81_DIR <- file.path(OUTPUT_DIR, "81_tet_mechanism")
dir.create(SEC81_DIR, recursive = TRUE, showWarnings = FALSE)

save_plot <- function(p, name, w = 10, h = 7) {
  save_multiformat_ggplot(p,
    base_path = file.path(SEC81_DIR, name),
...
panelA <- ggplot(panelA_summary,
  ...
  geom_col(fill = COLORS$methylation[["5hmC"]], width = 0.8) +
...
model_palette <- c(
  "TET impediment"     = COLORS$methylation[["5mC"]],
...
panelC <- ggplot(panelC_data,
  ...
  labs(
    ...
    x     = "Median delta demethylation ratio (mut - ctrl)",
    y     = NULL
  ) +
  theme_pub()
...
scatter_palette <- c("Other" = "grey75", "Neuronal" = COLORS$methylation[["5hmC"]])

slope_annot <- sprintf(
  "All genes: slope = %.3f (differs from -1)\nNeuronal: slope = %.3f (~-1)",
...
panelE_palette <- c("BAP1-KO" = COLORS$condition[["Mutant"]], "TET-KO" = "grey45")
...
panelE <- ggplot(panelE_data,
  ...
  labs(
    ...
    x     = NULL,
    y     = "Median delta demethylation ratio"
  ) +
  theme_pub()
...
#   design = "AABB\nCCDD\n#EE#", heights = c(1, 1, 0.6)
# =============================================================================
cat("\nComposing Section 81...\n")

layout_design <- "AABB\nCCDD\n#EE#"

sec81_composite <- panelA + panelB + panelC + panelD + panelE +
  plot_layout(design = layout_design, heights = c(1, 1, 0.6)) +
  plot_annotation(
    tag_levels = "A",
    title = "Section 81: TET-impediment, not DNMT3A recruitment",
    theme = theme(plot.title = element_text(face = "bold", size = 11, hjust = 0))
  )

cat("Saving Section 81 panels + composite...\n")

save_plot(panelA, "81a_dose_response", w = 8, h = 6)
save_plot(panelB, "81b_model_comparison", w = 8, h = 6)
save_plot(panelC, "81c_chromatin_state_lollipop", w = 8, h = 6)
save_plot(panelD, "81d_stoichiometry", w = 10, h = 8)
save_plot(panelE, "81e_tet_ko_attenuation", w = 6, h = 5)
save_plot(sec81_composite, "81_composite", w = 14, h = 12)

cat("\nSection 81 complete.\n")
cat("Output: ", SEC81_DIR, "\n")
