# scripts/figures/figureS1_supplemental.R
# =============================================================================
...
field_fill <- c(
  "Concordant"      = unname(PUB_COLORS$Hyper),
  "Discordant"      = unname(PUB_COLORS$Hypo),
...
panel_c <- ggplot(ctcf_df, aes(x = anchor, y = pct, fill = anchor)) +
  ...
  scale_fill_manual(
    values = c("Lost CTCF\nanchor" = unname(PUB_COLORS$Hyper),
               "Gained CTCF\nanchor" = unname(PUB_COLORS$Hypo)),
    guide = "none"
  ) +
...
cpgi_fill <- c("mC Hyper" = unname(PUB_COLORS$Hyper),
               "mC Hypo"  = unname(PUB_COLORS$Hypo),
               "Non-sig"  = "grey70")
...
panel_e <- ggplot(asm, aes(x = condition, y = sig_count)) +
  ...
  scale_fill_manual(values = c("Control" = unname(PUB_COLORS$Control),
                               "Mutant"  = unname(PUB_COLORS$Mutant)),
                    guide = "none") +
...
panel_f <- ggplot(tot, aes(x = mean_delta, y = group, colour = dir)) +
  ...
  scale_colour_manual(
    values = c("Decrease" = unname(PUB_COLORS$Hypo),
               "Increase" = unname(PUB_COLORS$Hyper)),
    name = "Total mC"
  ) +
  labs(
...
figureS1 <- panel_a + panel_b + panel_c + panel_d + panel_e + panel_f +
  plot_layout(design = design_s1, heights = c(1, 1, 1)) +
  plot_annotation(
    title = "Figure S1. Supplemental evidence: independent corroboration of the BAP1-KO methylation phenotype",
    tag_levels = "A",
    theme = theme(
...
save_figure(figureS1, "figureS1_supplemental", width_mm = 180, height_mm = 240)

# Also save each panel individually (useful for layout tuning / journal asks).
save_figure(panel_a, "figureS1_panelA_permutation",   width_mm = 110, height_mm = 90)
save_figure(panel_b, "figureS1_panelB_field_chr8",     width_mm = 90,  height_mm = 80)
save_figure(panel_c, "figureS1_panelC_ctcf_anchor",    width_mm = 80,  height_mm = 80)
save_figure(panel_d, "figureS1_panelD_cpgi_k119ub",    width_mm = 80,  height_mm = 80)
save_figure(panel_e, "figureS1_panelE_asm_doubling",   width_mm = 90,  height_mm = 80)
save_figure(panel_f, "figureS1_panelF_total_meth",     width_mm = 110, height_mm = 80)
