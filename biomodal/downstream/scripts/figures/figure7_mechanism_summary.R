# scripts/figures/figure7_mechanism_summary.R
# =============================================================================
...
panelA_cols <- c(
  ...
  "mC hyper genes"            = PUB_COLORS$Hyper,                   # "#D7191C"
  "mC hypo genes"             = PUB_COLORS$Hypo,                    # "#2C7BB6"
  "hmC genes"                 = "grey60"
)
...
p_B <- ggplot(logit,
  ...
  geom_errorbarh(aes(xmin = or_lower, xmax = or_upper),
                 height = 0.25, linewidth = 0.4, colour = "black") +
...
panelC_cols <- c("TET pathway"    = PUB_COLORS$hmC,   # 5hmC blue = TET pathway
                 "DNMT3A pathway" = PUB_COLORS$mC)    # 5mC red  = de-novo path
...
panelD_cols <- c(
  "CG only"               = PUB_COLORS$mC,                      # methylation red
...
fig7 <- fig7 +
  patchwork::plot_annotation(
    tag_levels = "A",
    title    = "Figure 7. Quantitative cascade: BAP1 loss → H2AK119ub → TET-block → methylation → MeCP2 → neuronal genes",
...
save_figure(fig7, "figure7_mechanism_summary",
            width_mm = 180, height_mm = 140)

save_figure(p_B, "figure7_panelB_k119ub_or_waterfall",
            width_mm = 90, height_mm = 70)

save_figure(p_D, "figure7_panelD_mecp2_r2_cascade",
            width_mm = 90, height_mm = 70)

cat("\nFigure 7 complete. Outputs in plots/figures/figure7_mechanism_summary/\n")
